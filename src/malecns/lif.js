...
export class LIFNetwork {
  ...
  step(){
    ...
    // Cache each neuron's state locally; preserve Float32 rounding at the same
    // intermediate updates as the reference kernel and preserve spike/RNG order.
    for(let n=0;n<count;n++){
      const i=list[n],di=drive[i],be=biasE[i],bi=biasI[i];
      let vi=v[i],ri=refr[i],ad0=adapt[i],tr0=trace[i],spiked=false;
      if(ri>0){ri=Math.fround(ri-dt);vi=vReset;}
      else if(di>0&&rng()<di*dtS){spiked=true;}
      else{vi+=(vRest-vi+gE[i]+gI[i]+be-bi)*kM;if(vi>=vThresh+ad0)spiked=true;}
      if(spiked){vi=vReset;ri=Math.fround(tRef);fired.push(i);spikeCount[i]++;tr0=1;ad0=Math.fround(ad0+adaptInc);}
      const e=gE[i]*dE,ii=gI[i]*dE,tr=tr0*dTr,ad=ad0*dA,rs=res[i]+(1-res[i])*kRec;
      refr[i]=ri;
      if(ri<=0&&di===0&&be===0&&bi===0&&Math.abs(vi-vRest)<1e-3&&e<1e-4&&-ii<1e-4&&tr<2e-3&&ad<1e-3&&rs>.999){v[i]=vRest;gE[i]=gI[i]=trace[i]=adapt[i]=0;res[i]=1;awake[i]=0;}
