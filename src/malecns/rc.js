const truncated=()=>{throw new Error('Truncated arithmetic payload');};
...
export class Decoder {
  ...
  next() { const p = this.s[2]; this.s[2] = p + 1; return p < this.b.length ? this.b[p] : truncated(); }
  bit(p, i) {
    ...
    if ((range >>> 24) === 0) { const buf = this.b; let pos = s[2]; do { range <<= 8; code = (code << 8) | (pos < buf.length ? buf[pos++] : truncated()); } while ((range >>> 24) === 0); s[2] = pos; }
  ...
  direct(_, nbits) {
    ...
    while (nbits > 0) {
      ...
      while ((range >>> 24) === 0) { range <<= 8; code = (code << 8) | (pos < buf.length ? buf[pos++] : truncated()); }
    }
...
UInt.prototype.dec = function (d, ctx) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    while ((range >>> 24) === 0) { range <<= 8; code = (code << 8) | (pos < len ? buf[pos++] : truncated()); }
  }
  const k = t - 32; let r = 1;
  if (k > 0) {
  ...
  st[0] = range; st[1] = code; st[2] = pos; return r - 1;
};
